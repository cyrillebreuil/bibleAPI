...
('"'"'cuv'"'"', '"'"'Chinese Union Version'"'"', '"'"'Chinese'"'"', '"'"'zho'"'"', '"'"'CN'"'"')
...
(FALSE, (SELECT "id" FROM "translations" WHERE "code" = '"'"'cuv'"'"'), '"'"'旧约'"'"'),
(TRUE, (SELECT "id" FROM "translations" WHERE "code" = '"'"'cuv'"'"'), '"'"'新约'"'"')
...
books_response=$(fetch_data "https://bible-api.com/data/cuv")
...
echo "$books" | while IFS= read -r line; do
    ...
    ('$book_id', (SELECT \"id\" FROM \"translations\" WHERE \"code\" = 'cuv'), '$book_name')
    ...
    echo "$chapters" | while IFS= read -r chapter_line; do
        ...
        echo "$chapter_response" | jq -r --arg book_id "$book_id" --arg chapter_number "$chapter_number" --arg translation_code "cuv" '
